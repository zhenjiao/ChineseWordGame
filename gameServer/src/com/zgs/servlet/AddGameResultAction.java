...
import com.zgs.util.TraceUtilities;
...
public class AddGameResultAction extends HttpServlet {
	...
	static Logger logger = Logger
			.getLogger(AddGameResultAction.class.getName());
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			// Get the rank from the b+ tree in memory, sort by score, level and
			// time
			// If > 10000, then drop the result and return 10000+
			// Else, if < 100, check if there is no data inserted into db (by
			// timestamp)
			// If yes, load the top 100 scores from db, to get accurate rank in
			// top 100 scores
			// Insert the new score to memory b+ tree
			// enqueue the new score (top 100 go to high pri queue, others go to
			// low pri queue)
			GameResultDAO game = new GameResultDAO();
			int rank = game.add(result);
			if (rank < 0) {
				String message = String
						.format("Got rank<0 when adding game record to DB.\n parameters are %s",
								paramString);
				out.print(message);
				logger.error(TraceUtilities.BuildLogMessageForHttpRequest(
						request, message));
			} else {
				out.print(String.format("Success. Rank = %d", rank));
				String message = String
						.format("Success. Rank = %d, Parameters are %s", rank, paramString);
				logger.debug(TraceUtilities.BuildLogMessageForHttpRequest(
						request, message));
			}
		} catch (Exception e) {
			String message = String
					.format("Got unhandled exception %s in AddGameResultAction.\n Parameters are %s",
							e.toString(), paramString);
			out.print(String.format(message));
			logger.error(TraceUtilities.BuildLogMessageForHttpRequest(request,
					message));
		} finally {
			out.flush();
			out.close();
